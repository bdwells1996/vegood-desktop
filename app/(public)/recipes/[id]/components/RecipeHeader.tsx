...
import type { RatingSummary, RecipeDetail } from "@/db/queries/recipes";
				...
				{avgRating !== null && (
					<div className="flex items-center gap-2">
						...
						<span className="text-sm text-content-tertiary">
							{avgRating.toFixed(1)} ({ratingSummary.count.toLocaleString()}{" "}
							ratings)
						</span>
					</div>
				)}
				...
				<div className="flex flex-wrap gap-2 mt-1">
					{recipe.prepTimeMins != null && (
						<Badge
							variant="grey"
							icon={Clock}
							label={`Prep ${recipe.prepTimeMins} min`}
						/>
					)}
					{recipe.cookTimeMins != null && (
						<Badge
							variant="grey"
							icon={Flame}
							label={`Cook ${recipe.cookTimeMins} min`}
						/>
					)}
					...
					{recipe.cuisine && <Badge variant="grey" label={recipe.cuisine} />}
					{recipe.difficulty && (
